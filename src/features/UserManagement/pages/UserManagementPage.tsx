...
const UserManagementPage = () => {
  ...
  const filteredUsers = useMemo(() => {
    return users.filter(user => {
      ...
      const teamMatch = filters.team ? user.team === filters.team : true;
  ...
  const filterConfig = [
    ...
    { key: 'role' as keyof Filters, options: [{ value: '', label: 'All Roles' }, ...DUMMY_ROLES.map(r => ({ value: r.name, label: r.name }))] },
  ];
