                ...
                <div className="mt-2 border p-3 rounded-b-md max-h-48 overflow-y-auto custom-scrollbar">
                    {currentPermissions.length > 0 ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                            {/* Gunakan p.id (number) dan ubah ke string */}
                            {currentPermissions.map(p => (
                                <label key={p.id} className="flex items-center space-x-2 py-1">
                                    <input type="checkbox" checked={selectedPermissions.has(p.id.toString())} onChange={() => handleCheckboxChange(p.id.toString())} className="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500" />
